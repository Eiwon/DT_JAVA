...
public class ListMain01 {
	...
	public static void main(String[] args) {
		...
		// for-each()
		// for(원소타입 변수명 : 배열 또는 컬렉션){}
		for(int n : list2) { // Integer -> int 자동 형 변환
			System.out.println(n);
		}
		
		System.out.println();
		// ArrayList에서 특정 인덱스의 원소 변경 : set(index, value)
		list2.set(0, 1);
		list2.set(2, 1234);
		for(Integer n : list2) {
			System.out.println(n);
		...
		System.out.println();
		// ArrayList에서 특정 인덱스의 원소 삭제 : remove(index)
		list2.remove(1);
		System.out.println("리스트 크기 : " + list2.size());
		for(int i=0; i<list2.size(); i++) {
			System.out.println("idx : " + i + ", value : " + list2.get(i));
		}
